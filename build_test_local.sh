...
if [[ $* == *--no-mock* ]]; then
  ...
  export DOCKER_FILES="$DOCKER_FILES -f docker-compose.local.no-mock.yml"
  ...
  export DOCKER_FILES="-$DOCKER_FILES -f docker-compose.local.yml -f docker-compose.mtls.yml"
else
